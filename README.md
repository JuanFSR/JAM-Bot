<h1 align="center">
  <img src="imgs/logo.png" alt="Logo do JAM Bot" width="120">
  <br>
 	JAM Bot - Just an Automated Marketplace
  <br>
</h1>

<h4 align="center">Um chatbot que auxilia no processo de venda e compra</h4>

<p align="center">
  <a href="https://www.python.org/">
    <img src="https://img.shields.io/badge/Python-3.8-FFDE57?style=for-the-badge&logo=Python"
         alt="Python">
          </a>
  <a href="https://rasa.com/">
    <img src="https://img.shields.io/badge/Rasa-3.X-6870EE?style=for-the-badge&logo=data:image/png;base64,"
         alt="Rasa">
          </a>
    
</p>

## Descrição
O JAM é um chatbot que ajuda comerciantes e compradores com seus anúncios e pesquisas. Automatiza o processo de vendas desenvolvendo um anúncio baseado em um questionário de produto e distribuindo-o nas plataformas onde está disponível, deixando o vendedor sem preocupações até a entrega da mercadoria. Para o cliente, simplifica o processo de busca e seleção de itens, filtrando com base nas preferências do usuário e determinando a melhor alternativa para eles.

## Funcionalidades

* Cadastra um produto
* Distribui o anúncio
* Integra o processo de venda e o processo de compra
* Busca um produto
* Filtra os anúncios
* Responde questões sobre o produto

## Como executar

### Requisitos

* Python 3.7 ou 3.8
* Rasa 3.X

#### Python

```
$ sudo apt install python3
```

#### Instalar dependências

```
$ pip3 install -r requirements.txt
```

---
